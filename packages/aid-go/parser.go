...
func ValidateRecord(raw map[string]string) (AidRecord, error) {
	...
	rec := AidRecord{V: v, URI: uri, Proto: proto}
	if auth, ok, err := getAliased("auth", "a"); err != nil {
		return AidRecord{}, err
	} else if ok {
		rec.Auth = auth
	}
	if desc, ok, err := getAliased("desc", "s"); err != nil {
		return AidRecord{}, err
	} else if ok {
		rec.Desc = desc
	}
	if docs, ok, err := getAliased("docs", "d"); err != nil {
		return AidRecord{}, err
	} else if ok {
		rec.Docs = docs
	}
	if dep, ok, err := getAliased("dep", "e"); err != nil {
		return AidRecord{}, err
	} else if ok {
		rec.Dep = dep
	}
	if pka, ok, err := getAliased("pka", "k"); err != nil {
		return AidRecord{}, err
	} else if ok {
		rec.Pka = pka
	}
	if kid, ok, err := getAliased("kid", "i"); err != nil {
		return AidRecord{}, err
	} else if ok {
		rec.Kid = kid
	}
	if rec.V == SpecVersionV1 && rec.Pka != "" && rec.Kid == "" {
		return AidRecord{}, newAidError("ERR_INVALID_TXT", "kid is required when pka is present")
	}
	if rec.V == SpecVersionV2 {
		if rec.Kid != "" {
			return AidRecord{}, newAidError("ERR_INVALID_TXT", "kid/i is not allowed in aid2 records")
		}
		if rec.Pka != "" {
			if err := validateAid2Pka(rec.Pka); err != nil {
				return AidRecord{}, err
			}
		}
	}
...
func validateAid2Pka(value string) error {
	bytes, err := decodeUnpaddedBase64URL(value)
	if err != nil {
		return newAidError("ERR_INVALID_TXT", "aid2 pka must be valid unpadded base64url")
	}
	if len(bytes) != aid2PkaPublicKeySize {
		return newAidError("ERR_INVALID_TXT", "aid2 pka must decode to exactly 32 bytes")
	}
	return nil
}

func decodeUnpaddedBase64URL(value string) ([]byte, error) {
	if value == "" {
