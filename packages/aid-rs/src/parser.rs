...
use crate::constants_gen::{
    AUTH_APIKEY, AUTH_BASIC, AUTH_CUSTOM, AUTH_MTLS, AUTH_NONE, AUTH_OAUTH2_CODE,
    AUTH_OAUTH2_DEVICE, AUTH_PAT, LOCAL_URI_SCHEMES, PROTO_A2A, PROTO_GRAPHQL, PROTO_GRPC,
    PROTO_LOCAL, PROTO_MCP, PROTO_OPENAPI, PROTO_UCP, PROTO_WEBSOCKET, PROTO_ZEROCONF,
    SPEC_VERSION_V1, SPEC_VERSION_V2, SUPPORTED_SPEC_VERSIONS,
};
...
fn validate_aid2_pka(value: &str) -> Result<(), AidError> {
    ...
    if decoded.len() != 32 {
        return Err(AidError::invalid_txt(
            "aid2 pka must decode to exactly 32 bytes",
        ));
    }
    Ok(())
}
...
pub fn parse(txt: &str) -> Result<AidRecord, AidError> {
    ...
    for raw_pair in txt.split(';') {
        ...
        let key_raw = iter
            .next()
            .ok_or_else(|| AidError::invalid_txt("Invalid key-value pair"))?;
        let value_raw = iter
            .next()
            .ok_or_else(|| AidError::invalid_txt(format!("Invalid key-value pair: {}", pair)))?;
        let key = key_raw.trim().to_lowercase();
        let value = value_raw.trim().to_string();
        if key.is_empty() || value.is_empty() {
            return Err(AidError::invalid_txt(format!(
                "Empty key or value in pair: {}",
                pair
            )));
        }
        match key.as_str() {
            "v" | "uri" | "u" | "proto" | "p" | "auth" | "a" | "desc" | "s" | "docs" | "d"
            | "dep" | "e" | "pka" | "k" | "kid" | "i" => {
                if seen.contains(&key) {
                    return Err(AidError::invalid_txt(format!("Duplicate key: {}", key)));
                }
        ...
        match key.as_str() {
            ...
            "auth" | "a" => {
                if auth.is_none() {
                    ...
                    return Err(AidError::invalid_txt(
                        "Cannot specify both \"auth\" and \"a\"",
                    ));
                }
            }
            "desc" | "s" => {
                if desc.is_none() {
                    ...
                    return Err(AidError::invalid_txt(
                        "Cannot specify both \"desc\" and \"s\"",
                    ));
                }
            }
            "docs" | "d" => {
                if docs.is_none() {
                    ...
                    return Err(AidError::invalid_txt(
                        "Cannot specify both \"docs\" and \"d\"",
                    ));
                }
            }
            "dep" | "e" => {
    ...
    if proto.is_some() && p.is_some() {
        return Err(AidError::invalid_txt(
            "Cannot specify both \"proto\" and \"p\" fields",
        ));
    }
    if proto.is_none() && p.is_none() {
        return Err(AidError::invalid_txt(
            "Missing required field: proto (or p)",
        ));
    }

    let proto_value = proto.or(p).unwrap();

    if !is_supported_proto(&proto_value) {
        return Err(AidError::unsupported_proto(format!(
    ...
    if let Some(ref d) = desc {
        if d.len() > 60 {
            return Err(AidError::invalid_txt(
                "Description field must be ≤ 60 UTF-8 bytes",
            ));
        }
    }
    ...
    if let Some(ref dv) = docs {
        if !dv.starts_with("https://") {
            return Err(AidError::invalid_txt(
                "docs MUST be an absolute https:// URL",
            ));
        }
    ...
    if proto_value == PROTO_LOCAL {
        ...
        let allowed = LOCAL_URI_SCHEMES.contains(&scheme);
        if !allowed {
        ...
        if !uri.starts_with("https://") {
            return Err(AidError::invalid_txt(format!(
                "Invalid URI scheme for remote protocol '{}'. MUST be 'https:'",
                proto_value
            )));
        }
    }
    ...
    if v == SPEC_VERSION_V2 {
        if kid.is_some() {
            return Err(AidError::invalid_txt(
                "kid/i is not allowed in aid2 records",
            ));
        }
...
mod tests {
    ...
    fn parses_aid1_and_aid2_pka_shapes() {
        let aid1 =
            parse("v=aid1;u=https://example.com/mcp;p=mcp;k=z6Mkf9;i=g1").expect("aid1 parses");
        assert_eq!(aid1.v, SPEC_VERSION_V1);
        assert_eq!(aid1.pka.as_deref(), Some("z6Mkf9"));
        assert_eq!(aid1.kid.as_deref(), Some("g1"));

        let aid2 = parse(&format!(
            "v=aid2;u=https://example.com/mcp;p=mcp;k={}",
            V2_PKA
        ))
        .expect("aid2 parses");
    ...
    fn aid2_rejects_kid_aliases_while_aid1_requires_kid() {
        for key in ["kid", "i"] {
            let raw = format!(
                "v=aid2;u=https://example.com/mcp;p=mcp;k={};{}=g1",
                V2_PKA, key
            );
