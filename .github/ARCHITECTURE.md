...
├── aid-go/               # Go SDK (source-only until module repo/tags are published)
├── aid-rs/               # Rust SDK (source-only; crates.io name not controlled yet)
├── aid-dotnet/           # .NET SDK (source-only until NuGet metadata/release is ready)
├── aid-java/             # Java SDK (source-only until Maven metadata/release is ready)
